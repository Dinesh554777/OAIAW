    ...
    return Object.entries(node).map(([key, value]) => {
      ...
      if (isDir) {
        ...
        return (
          <div key={currentPath}>
            <div 
              className="flex items-center gap-1.5 py-1 px-2 hover:bg-surface-elevated cursor-pointer text-foreground text-sm font-medium transition-colors rounded-sm mx-1 mt-0.5"
            ...
            {isExpanded && <div className="ml-4 border-l border-border pl-1.5">{renderTree(value, currentPath)}</div>}
          </div>
        );
      }
      ...
      return (
        <div 
          key={currentPath}
          className={`flex items-center gap-2 py-1 px-4 cursor-pointer text-sm font-mono rounded-sm mx-1 mt-0.5 transition-colors ${activeFile === currentPath ? 'bg-primary/10 text-accent' : 'text-muted-foreground hover:bg-surface-elevated hover:text-foreground'}`}
    ...
    <div className="h-full flex flex-col bg-surface border-r border-border shadow-sm">
      ...
      <div className="p-2 border-b border-border">
        <div className="relative">
          ...
          <input type="text" placeholder="Search files..." className="w-full bg-surface-elevated border-none text-xs rounded pl-7 py-1.5 text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-border" />
        </div>
      </div>
