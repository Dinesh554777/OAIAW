...
def get_auth_token(role: str):
    client.post("/auth/register", json={"name": "Test", "email": f"{role}@report.com", "password": "pass", "role": role})
    res = client.post("/auth/login", json={"email": f"{role}@report.com", "password": "pass"})
...
def test_unified_report():
    ...
    assert report["session"]["status"] == "SUBMITTED"
    assert "evaluation" in report
    assert "evidence" in report
    assert "events" in report
    assert "git_history" in report
